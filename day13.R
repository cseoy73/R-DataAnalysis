...
iris %>% pull(Species) 
iris %>% select(Species)
str(iris %>% pull(Species)) # factor
str(iris %>% select(Species)) # dataframe
...
# 집단별로 요약하기
exam %>%
  group_by(class) %>% summarise(n = n()) # 그룹(class)단위로 행의 개수를 세어줌

exam %>%
  group_by(class) %>% tally()   # 그룹(class)단위로 행의 개수를 세어줌
