...
bind_rows(group_a, group_b)  # 행으로 합치기
bind_cols(group_a, group_b)  # 열로 합치기(열 이름이 같으면 자동으로 이름 구분)
...
inner_join(test1, test2, by = "no")  # 공통인 것만 join
left_join(test1, test2, by = "no")  # 왼쪽(test1)에 join
right_join(test1, test2, by = "no")  # 오른쪽(test2)에 join
full_join(test1, test2, by = "no")  # 모두 join

merge(test1, test2)  #공통인 것만 merge
merge(test1, test2, all.x = T)   # 왼쪽(test1)에 merge
merge(test1, test2, all.y = T)   # 오른쪽(test2)에 merge
merge(test1, test2, all = T)    # 모두 merge
...
table(is.na(exam))                            # 결측치 있는지 확인 - 없음
...
boxplot(mpg$hwy, range=0) # 이상점까지 범위로
summary(mpg$hwy)
#상자그림 통계치 출력
...
na_count <-apply(x, 2, FUN=col_na)   # 2 -> 열별로 FUN 적용
na_count
...
# complete.cases 는 is.na 와 T/F가 반대
head(x)
...
# 5열 제거하고 Species로 그룹화, 각 그룹별로 mean함수 적용
...
table(mtcars$cyl)
summary(mtcars$cyl)
summary(factor(mtcars$cyl))   #factor로 만들면 summary하면 빈도수 나타남
ggplot(mtcars, aes(x=cyl)) + geom_bar()
ggplot(mtcars, aes(x=cyl)) + geom_bar(width=0.5)
ggplot(mtcars, aes(x=cyl)) + geom_bar(aes(fill=factor(gear)), alpha=0.5)   #alpha: 투명도
...
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+geom_point() + scale_color_brewer(palette = 'Set1')
# 범례 행 갯수 설정
