...
# 그래프의 작성
barplot(ds, main='인구 추정') # 기본적으로는 누적된 그래프 형태
...
par(mfrow=c(1, 1), mar=c(5, 5, 5, 7)) # 범례 위치 설정
barplot(ds, main='인구 추정', 
        col=c('green','blue','yellow'),
        beside=TRUE,
        legend.text=T,
        args.legend = list(x='topright', bty='n', inset=c(-0.75,0)))
# 원래는 insect=c(-0.25,0) 이다
...
barplot(ds, main='인구 추정', 
        col=c('green','blue','yellow'),
        beside=TRUE,
        legend.text=c('0~14세','15~64세','65세 이상'),  # 범례 안의 이름 바꾸기
