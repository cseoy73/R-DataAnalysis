...
mat1; dim(mat1)
matrix(chars, nrow=1)
matrix(chars, nrow=5) #default는 한열에 먼저 배분
matrix(chars, nrow=5, byrow=T) #행에 먼저 배분하는 식
matrix(chars, ncol=5)
...
vec2 <- c(4,5,6)
vec3 <- c(7,8,9)
mat1 <- rbind(vec1,vec2,vec3); mat1 #행으로 합치기
mat2 <- cbind(vec1,vec2,vec3); mat2 #열로 합치기
mat1[1,1]
mat1[2,];mat1[,3]
mat1[1,1,drop=F] #행과 열이름을 빼지 않음
...
fruit[-1,]
fruit[,2]
fruit[,3]  #가로로 표시됨, 데이터프레임 형식 없음
fruit[,3, drop=F] #(세로로 표시됨, 데이터프레임 형식 유지)
fruit[, c(3,4)]
fruit[3,2]
fruit[3,1]
...
summary(emp)
#factor로 바꾸니까 빈도수가 자동 계산(level에 따른 개수)
