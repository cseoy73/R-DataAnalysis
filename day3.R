...
a[[1]][2] <- 100
a
new_a <- unlist(a[1]) #named list였기 때문에 자동으로 naming이 됨
new_a
new_a[1]
names(a) <- NULL
a
...
#콘솔 화면에 데이터를 출력하는 함수
# print() : 데이터셋 또는 데이터를 출력
# cat() : 메시지를 출력할 때(paste를 사용하지 않아도 자동으로 하나의 문자열로 연결)
print(100)
print(pi)
data <- "가나다"
print(data)
...
for(data in month.name) 
  print(data)
for(data in month.name)print(data);print("hi")
for(data in month.name){print(data);print("hi")}
...
num <- sample(1:10,1)
num
switch(EXPR = num,"A","B","C","D") #4보다 큰 수가 나오면 출력되는 것 없음
...
# numeric일 때는 기본값 줄 수 없기 때문에 character로 변경
...
# break : 만족할 때 반복문 그만
# next : continue와 비슷, 만족할 때 빼고 나머지는 정상적으로 수행
sum <- 0
for(i in 5:15){
  if(i%%10==0){
    break
