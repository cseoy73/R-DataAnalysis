...
############## 스크롤(필요시): 5초 시간텀을 주고 한 경우 371번쯤 될때 쯤 3일이 지났다.
for (x in 1:500) {
   for (y in 1:3){
...
# 첫 페이지 클릭(처음 할 때만)
boardcss <- paste0('#react-root > section > main > article > div:nth-child(3) > div > div:nth-child(1) > div:nth-child(1) > a > div > div._9AhH0')
board1 <- remDr$findElement(using = 'css', boardcss)
board1$clickElement()



# 다시 할 때는 여기부터만(게시물 1000개씩 끊어서 저장)
for (i in 0:1000){
  # id
  ...
  # 다음 버튼
  nextbtn <- remDr$findElement(using='css selector','body > div._2dDPU.CkGkG > div.EfHg9 > div > div > a._65Bje.coreSpriteRightPaginationArrow')
...
insta11 <- rbind(insta11, df)

View(insta11)   

write.csv(insta11, "insta11.csv")

# 총 7434개의 데이터 추출, insta.csv로 저장